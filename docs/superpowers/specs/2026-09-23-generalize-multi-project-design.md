...
1. **관리자 AWS 키가 박스에 있고, 맥과 같은 키입니다.** 박스 `~/.aws/credentials`에 관리자 사용자의 액세스 키가 있고, 그 사용자의 키는 하나뿐이라 맥 CLI가 쓰는 키와 같습니다. 폰에서 여는 어느 세션이든 `cat`으로 읽을 수 있고(settings의 deny는 Read 도구만 막고 Bash는 못 막음), 트랜스크립트는 Anthropic 서버에 저장됩니다. 박스 파일만 지우면 이미 노출된 키가 살아 있으므로 **교체**가 필요합니다: 새 키 발급 → 맥 `~/.aws/credentials` 교체 → 옛 키 비활성화·삭제 → 박스 파일 삭제. 박스의 AWS 작업은 인스턴스 역할(`gamer4-operator` 정책)로 계속 됩니다.
2. **유휴 자동 정지가 동작하지 않습니다.** 대화가 없는 Remote Control 서버도 CPU를 분당 약 1초씩 써서, 유휴 감시가 5분마다 "활동"으로 판정합니다. 서버가 하나라도 떠 있으면 박스가 영원히 켜져 있습니다. 어젯밤 박스는 제가 알림 없이 정지해 두었습니다. 수정안은 §5-8.
...
| 5 | 관리자 키를 오늘 교체해도 되나 (위 1번) | 오늘 교체 |
...
3. v5 기능 유지: 전원, 상태, 세션 생성·정지·정리, 유휴 정지, 폰 승인 게이트, 푸시 알림.
...
- **AWS**: 관리자 키 교체(§0). 인스턴스 역할은 모든 프로젝트 작업의 **합집합 권한 하나**로 운영한다. 모든 서버가 같은 `ubuntu` 사용자·같은 인스턴스 역할로 돌기 때문에 프로젝트별 역할(F9)을 만들어도 한 세션이 다른 프로젝트 프로필을 쓸 수 있다. 프로젝트별 프로필은 **실수 범위를 줄이는 용도**일 뿐 보안 경계가 아니다. 필요할 때만 쓴다.
...
/home/ubuntu/.config/pocket-claude/
  telegram.env                         ← 부트봇 알림 (idle-watch 전용)
...
| 단계 | 내용 |
|---|---|
| **0. 즉시** | 관리자 키 교체(사용자), idle-watch CPU 기준 수정(§5-8) |
...
- 관리자 키를 "여부 결정"에서 "교체 절차"로. 맥과 같은 키임을 확인해 순서를 명시
