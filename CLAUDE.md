...
실제 값은 로컬 `.env`(gitignore), Lambda 환경변수, EC2의 `~/.config/gamer4/*.env`에만 둔다.
...
- `src/lambda_function.py` — 텔레그램 웹훅 Lambda (v5). 전원 스위치 + `claude-rc@<name>` 유닛 관리. Claude 기동에는 관여하지 않는다
- `ec2/` — systemd 템플릿 유닛, tmux 래퍼, sudoers, 유휴 감시, 설치 스크립트, settings·env 템플릿
- `tests/` — Lambda는 stdlib `unittest`, 셸은 bash 단정문 (macOS·Ubuntu 양쪽에서 돌아야 함)
...
- 테스트: `python3 -m unittest discover -s tests && bash tests/test_idle_watch.sh && bash tests/test_claude_rc_wrap.sh`
